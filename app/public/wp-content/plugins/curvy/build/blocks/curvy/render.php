<?php
    ...
    $topFlipX = $attributes['topFlipX'] ?? false;
    $topFlipY = $attributes['topFlipY'] ?? false;
    $topTransform = "scaleX(" . ($topFlipX ? "-1" : "1") . ") rotate(" . ($topFlipY ? "180deg" : "0") . ") scaleY(" . ($topFlipY ? "-1" : "1") . ")";
    $bottomFlipX = $attributes['bottomFlipX'] ?? false;
    $bottomFlipY = $attributes['bottomFlipY'] ?? false;
    $bottomTransform = "scaleX(" . ($bottomFlipX ? "-1" : "1") . ") rotate(" . ($bottomFlipY ? "180deg" : "0") . ") scaleY(" . ($bottomFlipY ? "-1" : "1") . ")";
?>
<div <?php echo $block_wrapper_attributes; ?>>
    <div class="curve top-curve" 
        style="
            display: <?php echo $attributes['enableTopCurve'] ?? false ? 'block' : 'none'; ?>; 
            transform: <?php echo $topTransform; ?>; 
            height: <?php echo $attributes['topHeight'] ?>px"
    >
        <svg
    ...
    <div class="curve bottom-curve" style="display: <?php echo $attributes['enableBottomCurve'] ?? false ? 'block' : 'none'; ?>; transform: <?php echo $bottomTransform; ?>; height: <?php echo $attributes['bottomHeight'] ?>px">
        <svg
            ...
            preserveAspectRatio="none" viewBox="0 0 1200 120"
        >
            <path d="<?php echo $bottomFlipY ? $invertedPath : $normalPath; ?>" fill="<?php echo $attributes['bottomColor'] ?? "white" ?>"></path>
        </svg>
    </div>
</div>